...
function render(){
 $('#project-list').innerHTML=d.projects.map((x,i)=>`<article class="row"><div class="status">${x.status}<br>${x.period}</div><div><h3>${x.title}</h3><p>${x.summary}</p></div><a href="#proyectos" aria-label="Ver ficha">Ver ficha →</a></article>`).join('');
 ...
 $('#member-list').innerHTML=d.members.map(x=>`<article class="person"><h3>${x.name}</h3><small>${x.degree}</small><p>${x.profile}</p></article>`).join('');
 ...
 $('#evidence-list').innerHTML=d.evidence.length?d.evidence.map(x=>`<div class="simple-item"><b>${x.title}</b><small>${x.date}</small></div>`).join(''):`<div class="simple-item"><b>Información pendiente de incorporar</b><small>Las evidencias se mostrarán con actividad, fecha, participantes y proyecto relacionado.</small></div>`;
}
render();
...
$('#search').addEventListener('input',e=>{const q=e.target.value.toLowerCase().trim();const all=[...d.projects.map(x=>({...x,kind:'Proyecto'})),...d.products.map(x=>({...x,kind:'Producción'})),...d.members.map(x=>({title:x.name,summary:x.profile,kind:'Integrante'})),...d.blog.map(x=>({...x,kind:'Blog'})),...d.evidence.map(x=>({...x,kind:'Evidencia'}))];const r=q?all.filter(x=>JSON.stringify(x).toLowerCase().includes(q)):[];$('#searchResults').innerHTML=r.length?r.map(x=>`<div class="search-result"><small>${x.kind}</small><b>${x.title}</b><div>${x.summary||''}</div></div>`).join(''):(q?'<p>No se encontraron coincidencias.</p>':'')});
